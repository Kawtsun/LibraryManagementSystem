...
/**
 * Inserts the transaction record (saving the book title) into the transactions table,
...
function insertTransaction($conn, $data) {
    ...
    $book_title    = $data['book_title']; // To be stored in the transaction record
    $date_borrowed = $data['date_borrowed'];
    $return_date   = $data['return_date'];
    $course        = $data['course'];
    $author        = $data['author'];
    $source        = $data['source'];
    $book_id       = $data['book_id']; // Numeric id used for updating available count
    ...
    try {
        ...
        // 2. Update the available count for the corresponding book record using its numeric id.
        switch ($source) {
            case 'books':
