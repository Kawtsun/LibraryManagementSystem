...
<div class="container">
    <section class="book-section">
        ...
        <div class="book-grid">
            <?php
            if (!empty($authors)) {
                $authorCounter = 1; // Initialize the counter
                foreach ($authors as $author) {
                    $filename = 'author_detail.php?author=' . urlencode($author);
                    $imageFilename = 'author' . $authorCounter . '.png';
                    $imagePath = 'authors_images/' . $imageFilename;

                    // Check if the image file exists
                    if (!file_exists($imagePath)) {
                        $imagePath = 'authors_images/default_author.png'; // Use default image if not found
                    }

                    echo '<div class="book-item">';
                    echo '<img src="' . $imagePath . '" alt="' . $author . ' Icon" class="book-icon">';
                    echo '<p class="book-title">' . $author . '</p>';
                    echo '<a href="' . $filename . '" class="see-books-btn">SEE BOOKS</a>';
                    echo '</div>';

                    $authorCounter++; // Increment the counter
                    if ($authorCounter > 21) {
                        $authorCounter = 1; // Reset counter if it exceeds 21
                    }
                }
            } else {
